...
// Callback za pomeranje kamere misom
...
int main() {
    ...
    TexturedModel texturedModel = TexturedModel(rawModel, texture);     // TexturedModel Represent Model With RawModel and Texture
    ...
    // Load Terrain Textures - MultiTexturing
   ...
   for(j = 0; j < 200; j++) {
        ...
        // Generate Random Grass Entities
        if(c < 100) {
    ...
    float skyboxVertices[] = {      // Sky box Vetices
        // positions
        -1.0f,  1.0f, -1.0f,
        -1.0f, -1.0f, -1.0f,
         1.0f, -1.0f, -1.0f,
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Proces Entities - add to Render List
        for(i = 0; i < entities.size(); i++) {
            masterRenderer.processEntity(entities.at(i));
        }


        // Add Terrain to Terrein Render List
        ...
        model = glm::scale(model, glm::vec3(400000.0f));
...
float yaw = -90.0f;  // Gore Dole ugao
float picth = 0.0f;  // Levo Desno ugao

// Pomeramo Kameru Misem
void mouse_callback(GLFWwindow* window, double xpos, double ypos) {
